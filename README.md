...
- **后台轨迹补绘**：应用回到前台时会同步后台采集的轨迹点，避免只连接前后台两个端点。
- **定位信息面板**：实时显示经纬度、GPS 高程、气压高程、精度、速度和更新时间。
- **轨迹导入导出**：支持通过系统文件选择器导入和导出 GPX / GeoJSON 轨迹。
...
5. 使用轨迹节点开关控制轨迹采样点显示。
6. 使用右上角轨迹文件按钮导入或导出 **GPX / GeoJSON** 轨迹。
7. 点击 **🗑️ 清除标记** 清理当前地图上的定位标记与轨迹。
